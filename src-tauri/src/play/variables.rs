...
impl GameArguments {
  pub fn collect(&self) -> HashMap<String, String> {
    ...
    hashmap.insert("auth_access_token".to_string(), self.auth_access_token.clone());
    ...
    hashmap.insert("server_ip".to_string(), self.server_ip.clone().split(":").collect::<Vec<&str>>().first().unwrap().to_string());
